...
func runProject() *cobra.Command {
	return &cobra.Command{
		Use:     "run",
		Example: "t0 run",
		Short:   "Run Tndr0cean development server",
		Run: func(cmd *cobra.Command, args []string) {
			if _, err := os.Stat("cmd/main.go"); err != nil {
				fmt.Println("not in Tndr0cean app root: cmd/main.go not found")
				return
			}
			if err := exec.Command("t1", "generate").Run(); err != nil {
				fmt.Println(err)
				return
			}
...
func installProject() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err := exec.Command("go", "get", "github.com/senforsce/tndr0cean@latest").Run(); err != nil {
				fmt.Println(err)
				return
			}

			if err := exec.Command("go", "get", "github.com/senforsce/tndr").Run(); err != nil {
				fmt.Println(err)
				return
			}
			if err := exec.Command("t1", "generate").Run(); err != nil {
...
func generateProject() *cobra.Command {
	return &cobra.Command{
		Use:     "new",
		Example: "nw new hello-world",
		Short:   "Create new nw project",
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) == 0 {
			...
			fmt.Println("creating new nw project:", name)
			...
			files := []File{
				// setup directory
				{Path: name + "/Concepts/", Content: nil},
				{Path: name + "/Concepts/Example", Content: nil},
				{Path: name + "/Concepts/Example/layout", Content: nil},
				{Path: name + "/Concepts/Example/HelloWorld", Content: nil},
				{Path: name + "/Concepts/Example/handler", Content: nil},
				...
				{Path: name + "/Concepts/Example/handler/hello.go", Content: writeHandlerContent(name)},
				{Path: name + "/Concepts/Example/layout/base.t1", Content: writeBaseLayoutContent()},
				{Path: name + "/Concepts/Example/HelloWorld/view.t1", Content: writeViewContent(name)},
			}
			...
			if len(errors) != 0 {
				fmt.Println("Tndr0cean encountered errors during file initialization:", errors)
				return
			}
		},
	}
...
func generateConcept() *cobra.Command {
	return &cobra.Command{
		Use:     "concept",
		Example: "Tndr0cean concept user",
		Short:   "Generate new concept with ontology",
		Run: func(cmd *cobra.Command, args []string) {
			// TODO:
		},
	}
...
func generateHandler() *cobra.Command {
	return &cobra.Command{
		Use:     "handler",
		Example: "Tndr0cean handler home",
...
func OpenConnection() {
	...
	mysql := config{
		Hostname: os.Getenv("NW_SQL_DB_HOST"),
		Username: os.Getenv("NW_SQL_DB_USER"),
		Password: os.Getenv("NW_SQL_DB_PASSWORD"),
		Port:     os.Getenv("NW_SQL_DB_PORT"),
		DBName:   os.Getenv("NW_SQL_DB_NAME"),
	}
...
import (
	"log"
	"github.com/senforsce/tndr0cean"
	"%s/handler"
)
...
[build]
  args_bin = []
  bin = "./.build/main"
  cmd = "t1 generate && go build -o ./.build/main ./cmd"
  delay = 1000
  exclude_dir = ["assets", ".build", "vendor", "testdata"]
  exclude_file = []
  exclude_regex = ["_test.go", "_t1.go"]
  exclude_unchanged = false
  follow_symlink = false
  full_bin = ""
  include_dir = []
  include_ext = ["go", "tpl", "tmpl", "html", "t1"]
  include_file = []
  kill_delay = "0s"
